...
export function EditMessageModal(props: {
	idx: number;
	originalText: string;
	editMessageCallback: (idx: number, text: string) => void;
	visibilityCallback: (visible: boolean) => void;
}) {
	const [editedMessage, setEditedMessage] = createSignal(props.originalText);

	function toggleModal() {
		props.visibilityCallback(false);
	}
	function formSubmit(e: Event) {
		e.preventDefault();
		props.editMessageCallback(props.idx, editedMessage());
		toggleModal();
	}
	return (
		<Portal>
						...
						>
							<div class="flex justify-center space-x-2">
								...
								<input
									...
									type="text"
									value={editedMessage()}
									onInput={(e) => setEditedMessage(e.target.value)}
